...
void gestureSelectBank(uint8_t bank) {
    uint8_t buf[2];
...
void gestureWriteReg(uint8_t addr, uint8_t cmd) {
    uint8_t buf[2] = {addr, cmd};
    i2c_write(I2C1, PAJ7620_ID, buf, 2);
    return;
}

uint8_t gestureReadReg(uint8_t addr, uint8_t qty) {
    uint8_t buf[1] = {addr};
...
void print_gesture() {
    ...
    if (data == GES_RIGHT_FLAG) {
        ...
        if (data == GES_FORWARD_FLAG) {
            ...
            // uart_printf(_USART1, "Forward \r\n");
            delay_us(GES_QUIT_TIME);
        } else if (data == GES_BACKWARD_FLAG) {
            uart_printf(_USART2, "Backward\r\n");
            // uart_printf(_USART1, "Backward\r\n");
            delay_us(GES_QUIT_TIME);
        } else {
            uart_printf(_USART2, "Right\r\n");
            //uart_printf(_USART1, "Right\r\n");
        }
    } else if (data == GES_LEFT_FLAG) {
        delay_us(GES_ENTRY_TIME);
        data = gestureReadReg(0x43, 1);
        ...
        //uart_printf(_USART1, "Clockwise\r\n");
    } else if (data == GES_COUNT_CLOCKWISE_FLAG) {
        uart_printf(_USART2, "anti-clockwise\r\n");
         //uart_printf(_USART1, "anti-clockwise\r\n");
    } else {
        data = gestureReadReg(0x44, 1);
        if (data == GES_WAVE_FLAG) {
            uart_printf(_USART2, "wave\r\n");
            //uart_printf(_USART1, "wave\r\n");
        }
    }
}